...
@Composable
fun Ajustes(
    participante: ParticipanteData?, onCerrarSesion: () -> Unit, navController: NavController
) {
    // Para ir a la web
    ...
    var participante by remember { mutableStateOf(participante) }
    ...
    Column(
        ...
        Text(
            "Ajustes", style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.Bold
        )
            ...
            Card(
                ...
                Column(modifier = Modifier.padding(16.dp)) {
                    ...
                    Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.Center) {
                        ...
                        TextButton(
                            onClick = {
                                Firebase.auth.sendPasswordResetEmail(participante?.emailEduca ?: "")
                                    .addOnSuccessListener {
                                        Toast.makeText(context, "Email de restablecimiento enviado", Toast.LENGTH_SHORT).show()
                                    }
        ...
        Column(
            ...
            // Cerrar sesión
            TextButton(
                onClick = onCerrarSesion, colors = ButtonDefaults.textButtonColors(
            ...
            Card(
                ...
                Column(
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        Button(onClick = {
                            when {
                                nuevoEmail.isBlank() -> Toast.makeText(context, "Introduce el nuevo email", Toast.LENGTH_SHORT).show()
                                !emailPattern.matches(nuevoEmail) -> Toast.makeText(context, "Formato de email inválido", Toast.LENGTH_SHORT).show()
                                nuevoEmail == participante?.emailEduca -> Toast.makeText(context, "El email es el mismo que el actual", Toast.LENGTH_SHORT).show()
                                passwordActual.isBlank() -> Toast.makeText(context, "Introduce tu contraseña actual", Toast.LENGTH_SHORT).show()
                                else -> {
